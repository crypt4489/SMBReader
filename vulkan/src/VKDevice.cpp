...
static T_VkType GetVkTypeFromEntry(VKDevice* device, EntryHandle index)
{
...
static EntryHandle AddVkTypeToEntry(VKDevice* device, T_VkType handle)
{
...
void VKDevice::CreateQueueManager(QueueManager *manager, uint32_t queueIndex, uint32_t maxCount, uint32_t queueFlags, bool presentsupport)
{
	...
	void* queueManagerData = AllocTypeFromEntry<void*>(this, sizeof(size_t) * maxCount);

	std::construct_at(manager, std::vector<uint32_t>{}, maxCount, queueIndex, queueFlags, presentsupport, *this, queueManagerData);
}
...
EntryHandle VKDevice::CreateCommandPool(QueueIndex& queueIndex)
{
	...
	EntryHandle poolIndex = AddVkTypeToEntry(this, pool);

	return poolIndex;
}

VkDescriptorPool VKDevice::CreateDesciptorPool(EntryHandle& poolIndex, DescriptorPoolBuilder& builder, uint32_t maxSets)
{
	std::shared_lock lock(deviceLock);
	...
	poolIndex = AddVkTypeToEntry(this, descriptorPool);
...
EntryHandle VKDevice::CreateHostBuffer(VkDeviceSize allocSize, bool coherent, bool createAllocator, VkBufferUsageFlags usage)
{
	...
	{
		ret = AddVkTypeToEntry(this, buffer);

		EntryHandle memLoc = AddVkTypeToEntry(this, memory);
	}
	...
	EntryHandle hostIndex)
{
	...
	VkCommandPool pool = GetVkTypeFromEntry<VkCommandPool>(this, poolIndex);
...
{
	std::shared_lock lock(deviceLock);
	auto deviceMem = GetVkTypeFromEntry<VkDeviceMemory>(this, hostIndex + 1);;
	void* datalocal;
...
OffsetIndex VKDevice::GetOffsetIntoHostBuffer(EntryHandle hostIndex, size_t size, uint32_t alignment)
{
	...
	auto alloc = GetVkTypeFromEntry<VKAllocator*>(this, hostIndex + 2);
...
void VKDevice::UpdateRenderGraph(EntryHandle renderPass, std::vector<uint32_t>& dynamicOffsets, std::string perGraphDescriptor)
{
	std::shared_lock lock(deviceLock);
...
std::vector<EntryHandle> VKDevice::CreateSemaphores(uint32_t count)
{
	std::shared_lock lock(deviceLock);

	std::vector<EntryHandle> ret(count);
	...
	{	
		...
		ret[i] = AddVkTypeToEntry(this, sema);
	}

	return ret;
}

std::vector<EntryHandle> VKDevice::CreateFences(uint32_t count, VkFenceCreateFlags flags)
{
	std::shared_lock lock(deviceLock);
	std::vector<EntryHandle> ret(count);
	...
	for (uint32_t i = 0; i < count; i++) {
		...
		ret[i] = AddVkTypeToEntry(this, fence);
	}
...
uint32_t VKDevice::SubmitCommandsForSwapChain(EntryHandle swapChainIdx, uint32_t frameIndex, EntryHandle cbIndex)
{
	...
	std::vector<EntryHandle> dummie1{ (depends[0]) };

	std::vector<EntryHandle> dummie2{ (depends[1]) };
...
std::vector<EntryHandle> VKDevice::CreateReusableCommandBuffers( 
	uint32_t numberOfCommandBuffers, VkCommandBufferLevel level, bool createFences, uint32_t capabilites)
{
	...
	std::vector<EntryHandle> ret(numberOfCommandBuffers);
...
EntryHandle VKDevice::CreateFrameBuffer(std::vector<EntryHandle>& attachmentIndices, EntryHandle renderPassIndex, VkExtent2D& extent)
{
	std::shared_lock lock(deviceLock);
	...
	EntryHandle ret = AddVkTypeToEntry(this, frameBuffer);
...
RenderTarget& VKDevice::GetRenderTarget(EntryHandle renderTargetIndex)
{
	std::shared_lock lock(deviceLock);
...
QueueManager::QueueManager(std::vector<uint32_t> _cqs, int32_t _mqc, uint32_t _qfi, uint32_t _queueCapabilities, bool present, VKDevice& _d, void *data) :
	bitmap(0U),
	maxQueueCount(_mqc),
