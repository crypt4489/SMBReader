...
public:
	void CreateRenderInstance()
	{
		...
		for (auto& requested : instanceExtensions)
		{
			if (std::find_if(extensions.begin(), extensions.end(), [&requested](auto i) {
				return strcmp(requested, i.extensionName);
				}) == std::end(extensions))
			{
		...
		for (auto& requested : instance_validation_layers)
		{
			if (std::find_if(layerProps.begin(), layerProps.end(), [&requested](auto i) {
				return strcmp(requested, i.layerName);
				}) == std::end(layerProps))
			{
		...
		createInfo.ppEnabledLayerNames = instance_validation_layers.data();
		createInfo.enabledLayerCount = instance_validation_layers.size();
	...
	void DestroyRenderInstance()
	{
		...
		if (instDevice)
		{
			vkDestroyDevice(instDevice, nullptr);
		}
	}
private:
